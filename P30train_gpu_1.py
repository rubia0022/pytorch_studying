...
#创建网络模型
model=Model()
if torch.cuda.is_available():#判断电脑是否可用cuda，即是否有gpu
    model=model.cuda()#模型加入cuda
#损失函数
loss_fn=nn.CrossEntropyLoss()#交叉熵损失
if torch.cuda.is_available():#判断电脑是否可用cuda，即是否有gpu
    loss_fn=loss_fn.cuda()#损失加入cuda
#优化器
learning_rate=1e-2 #1*10^(-2)=0.01
optimizier=torch.optim.SGD(model.parameters(),lr=learning_rate)#SGD优化器
#设置训练网络的一些参数
...
for i in range(epoch):
    ...
    #训练步骤开始
    model.train()#很多算法中模型在训练之前会调用train函数
    for data in train_dataloader:
        imgs,targets=data
        if torch.cuda.is_available():  # 判断电脑是否可用cuda，即是否有gpu
            imgs=imgs.cuda()#input加入cuda
            targets=targets.cuda()#target加入cuda
        outputs=model(imgs)
    ...
    #测试步骤
    model.eval()#很多算法中模型在训练之前会调用eval函数
    ...
    #保存每epoch轮的模型
    torch.save(model,"model_{}.pth".format(i))#这里是方式一保存模型
    #方式二torch.save(model.state_dict(),"model_{}.pth".format(i))
    print("模型已保存")
writer.close()
